...
const BusinessArea = () => {
  ...
  const filteredArea = DUMMY_DATA.reduce((acc,cur) => {
  if(acc.findIndex(({country}) => country === cur.country) === -1 ){
     acc.push(cur)
    }
  return acc;
  },[])
	console.log(filteredArea[2].country.toLowerCase().split(' ').join(''))
  return (
    <>
    <Wrapper>
    ...
    <Route path={'/area/:memberarea'} exact>
      ...
      <AreaDetail filteredArea={filteredArea} dummy={DUMMY_DATA} />
			</Route>
